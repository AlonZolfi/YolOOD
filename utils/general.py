...
LOGGER = logging.getLogger("yolov5")  # define globally (used in train_yolood.py, val_yolood.py, detect.py, etc.)
...
def download(url, dir='.', unzip=True, delete=True, curl=False, threads=1, retry=3):
    ...
    def download_one(url, dir):
        ...
        if unzip and success and f.suffix in ('.zip', '.gz', '.rar'):
            ...
            if f.suffix == '.zip':
                ...
                # extract_archive(f, outdir=dir, program='rar')
            if delete:
                f.unlink()  # remove zip
...
def strip_optimizer(f='best.pt', s=''):  # from utils.general import *; strip_optimizer()
    ...
    for k in 'optimizer', 'best_fitness', 'wandb_id':  # keys
        x[k] = None
    x['epoch'] = -1
    # torch.save(x['model'].state_dict(), f.name + 'h')
